...
bool Kittler::FindThreshold(const arma::vec &H, double RHmin, double RHmax)
{
	...
	// stopping cases , threshold i threshold separtes bin i (including itself) from the rest
	P1[0] = H[0] ;
	P2[N-2] = H[N-1];
	Mu1[0] = 0;
	...
	for(int i=1, j=N-3; i <= N-2 ; i++, j--) {
		...
		if ( P1[i]!=0 ) {
			...
			Var1[i]= ( P1[i-1] * ( Var1[i-1] + (Mu1[i-1]-Mu1[i]) * (Mu1[i-1]-Mu1[i]) ) + H[i] * (i - Mu1[i]) * (i - Mu1[i]) ) / P1[i];
		} else
			Mu1[i]=0,Var1[i]=0;
		...
		if ( P2[j]!=0 ) {
			...
			Var2[j]= ( P2[j+1] * ( Var2[j+1] + (Mu2[j+1]-Mu2[j]) * (Mu2[j+1]-Mu2[j]) ) + H[j+1] * (j+1 - Mu2[j]) * (j+1 - Mu2[j]) ) / P2[j];
		} else
			Mu2[j]=0, Var2[j]=0;
	}
	...
	deque<bool> IsMin=MarkMinima(J);           // mark all local minima of the criterion function J
	
	//cout << "mark all local minima" << endl;
...
bool Kittler::FindGlobalMin(const deque<bool> &IsMin, const vector<double> &J)
{
	...
	while( LeftMin < IsMin.size() ) {
		...
		LocalMin=static_cast<double>( ( LeftMin + RightMin-1 ) )/ 2;


		//  Monotonically ascend to the left
		unsigned int LeftHeight=static_cast<int>(LocalMin);
		while(LeftHeight >=1 && J[LeftHeight-1] >= J[LeftHeight]) LeftHeight--;


		//  Monotonically ascend to the right
		unsigned int RightHeight=static_cast<int>(LocalMin);
		while(RightHeight < IsMin.size()-1 && J[RightHeight] <= J[RightHeight+1]) RightHeight++;
